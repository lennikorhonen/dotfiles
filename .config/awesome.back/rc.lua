...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "www", "dev", "sys", "doc", "chat", "mus",}, s, layouts[1])
end
-- }}}
...
vicious.register( batwidget, vicious.widgets.bat, '<span background="#92B0A0" font="mononoki"><span font="mononoki color="#FFFFFF" background="#92B0A0">$1$2% </span></span>', 30, "BAT0" )
...
'<span background="#D0785D" font="mononoki"> <span font="mononoki" color="#EEEEEE">${/home used_gb}/${/home avail_gb} GB </span></span>',
800)
...
-- {{ Spawns Discord }} --
    ...
    awful.key({ modkey }, "d", function() awful.util.spawn("com.discordapp.Discord")end,
...
--[[
	...
	awful.key({ modkey, "Control"}, "k", function () awful.screen.focus_relative( -1) end,
		{description="focuse the previous screen", group="screen"}),
    ...
    awful.key({ modkey, "Control" }, "n",
        function ()
            ...
            -- Focus restored clinet
            if c then
                c:emit_signal(
                    "request::activated", "key.unminimize", {raise=true}
                )
            end
        end,
    ...
    -- Prompt
    awful.key({ modkey },            "v",     function () awful.util.spawn("dmenu_run")end,
...
clientkeys = awful.util.table.join(
    awful.key({ modkey,           }, "f",      function (c) c.fullscreen = not c.fullscreen  end,
        {description="toggle fs", group="client"}),
...
clientbuttons = awful.util.table.join(
    ...
    awful.button({ modkey }, 3, function (c)
            c:emit_signal("request::acticate", "mouse_click", {raise=true})
